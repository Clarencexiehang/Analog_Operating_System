...
void Disk::releaseSpace(QString filename, QString policy)
{
    ...
    //获取文件磁盘块序列
    qDebug()<<" 查看 "<<policy<<" 文件 "<<" 磁盘块序列";
    QVector<int>cyIDorder;
    if(policy=="连续分配"){
        for (int i=0;i<fileBlocks.size();i++) {
            //对每个文件块，找里面的每个磁盘块
            for (int j=fileBlocks[i].firstBlockId;j<=fileBlocks[i].firstBlockId+fileBlocks[i].blockNum-1;j++) {
                cyIDorder.append(diskBlock[j]->blockId);
            }
        }
    }else if(policy=="显式链接"){
        //找到文件的块号
        ...
        while (BlockId!=-2) {
            cyIDorder.append(diskBlock[BlockId]->blockId);
            BlockId=Fat[BlockId].value;
        }
    }else{
        //索引文件目录中找到起始索引块
        ...
        while (indexBlockId!=-1) {
            ...
            for (int i=0;i<blockNum;i++) {
                if(indexFiles[indexBlockId].blocksID[i]==-1){
                    ...
                    qDebug()<<"磁盘块 ："<<indexFiles[indexBlockId].blocksID[i]<<" 磁道号："<<diskBlock[indexFiles[indexBlockId].blocksID[i]]->cyId;
                    cyIDorder.append(diskBlock[indexFiles[indexBlockId].blocksID[i]]->blockId);
                }
            }
            indexBlockId=indexFiles[indexBlockId].nextIndexBlockId;
        }
    ...
    qDebug()<<"清空磁盘块：";
    for (int i=0;i<cyIDorder.size();i++) {
        qDebug()<<cyIDorder[i];
        //清空磁盘块
        diskBlock[cyIDorder[i]]->fileId=-1;
        //清空磁盘块颜色
        int rr=(cyIDorder[i])/(ui->block_tab->columnCount());
        int cc=(cyIDorder[i])%(ui->block_tab->columnCount());
        QTableWidgetItem *tt=ui->block_tab->item(rr,cc);
